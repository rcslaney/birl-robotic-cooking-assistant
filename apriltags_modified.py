...
Richard Slaney's modified version that adds a "tag_size_overrides"
keyword argument to the detect function that lets you pass a dict
to specify tags with different sizes to the size in the "tag_size"
parameter.
...
class Detector(object):
    ...
    def detect(self, img, estimate_tag_pose=False, camera_params=None, tag_size=None, tag_size_overrides={}):
...
image of type numpy.uint8.'''
        ...
        for i in range(0, detections.contents.size):
            ...
            if estimate_tag_pose:
                ...
                if detection.tag_id in tag_size_overrides:
                    tag_size_to_use = tag_size_overrides[detection.tag_id]
                else:
                    tag_size_to_use = tag_size

                camera_fx, camera_fy, camera_cx, camera_cy = [ c for c in camera_params ]

                info = _ApriltagDetectionInfo(det=apriltag,
                                              tagsize=tag_size_to_use,
                                              fx=camera_fx,
                                              fy=camera_fy,
                                              cx=camera_cx,
                                              cy=camera_cy)
