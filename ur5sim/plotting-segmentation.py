...
armd = json.load(open("arm_trajectory_4.json", "r"))
trajd = json.load(open("trajectory_recording_4.json", "r"))
...
t1 = [p[0] - trajd["14"][0][0] + tshift for p in trajd["14"]]
x1 = [p[1][0] + xshift if p[1] is not None else np.nan for p in trajd["14"]]
...
t2 = np.linspace(0, len(x2) * 0.1, len(x2))
